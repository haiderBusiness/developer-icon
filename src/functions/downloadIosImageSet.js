...
const downloadIosImageSet = async (
  svgString = "",
  folderName,
  size,
  callBack = () => {
    console.log("you need to do the callback...this is just a test message");
  }
) => {
  const zip = new JSZip();
  ...
  for (let i = 0; i < svgStrings.length; i++) {
    ...
    const svgString = pureSvg
      .replace(/ width="[^"]*"/, ` width="${size * (i + 1)}"`)
      .replace(/height="[^"]*"/, `height="${size * (i + 1)}"`);
    ...
    zip.file(
      `${folderName}_imageset/${fileName}.png`,
      pngDataUri.split(";base64,").pop(),
      { base64: true }
    );

    if (i + 1 === svgStrings.length) {
      zip.file(
        `${folderName}_imageset/Contents.json`,
        JSON.stringify(generateJsonOjbect(folderName, size))
      );
    }
