...
const ExampleList = ({ divRef }) => {
  ...
  const name = iconSections.some(
    (iconSection) => iconSection.name === sectionName
  )
    ? sectionName
    : null;
  ...
  const searching =
    hash1.includes(searchingString) && hash1.length > searchingString.length;

  // console.log("window.location.hash: ")

  const [allIcons, setAllIcons] = useState([]);
  const [shortcut, setShortcut] = useState(
    shortcutSmallCase ? shortcutSmallCase : !searching ? "ai" : null
  );

  const { iconsToShow, searchInputValue, activeSection } = useSelector(
    (state) => state.reducer
  );
  ...
  useEffect(() => {
    console.log("called here#", activeSection);
    const shortcutSmallCase = activeSection
      ? getItemByName(iconSectionsArr, activeSection)
      : null;
  ...
  const handleResize = (
    ...
    if (div) {
      ...
      // obj.columnCount = 2
      obj.columnWidth = obj.divWidth / 1.032 / obj.columnCount;
        ...
        showLoading ? (
          ...
          >
            {({ columnIndex, rowIndex, style }) => (
              <Cell
                onClick={(objectReceived) => {
                  handleItemClicked(objectReceived);
                }}
                ...
                allIcons={allIcons}
                isSelectedItemIndex={
                  rowIndex * listAttributes.columnCount + columnIndex ===
                  selectedItemIndex
                }
                // dispath={dispatch}
              />
  ...
  return (
    <>
      ...
      <div className={styles.emptySupTitle}>
        Not finding an icon that you are sure of its existance here ?{" "}
        <a href="https://ionic.io">File an issue</a> We'll fix it as soon as
        possible.
      </div>
      <div
        onClick={() => onClick()}
...
function IconsList({ listWidth, style }) {
  ...
  return (
    ...
    >
      <div className={styles.listMainDiv}>{ExampleList({ divRef })}</div>
    </div>
  );
}
